...
namespace InnoEngine
{
    ...
    public:
        ...
        void build_pool( SizeType ObjectAmount )
        {
            ...
            IE_ASSERT( totalSize <= ( std::numeric_limits<SizeType>::max )() );
        ...
        void deallocate( void* objectDeleted ) noexcept
        {
            ...
            if constexpr ( PoolType == ObjectPoolType::LastOutFirstIn ) {
                ...
                *pNextFreeOffset  = m_firstFreeOffset;
                ...
                if ( pFreeBefore ) {
                    while ( offsetDeleted > pFreeBefore->NextFreeOffset && pFreeBefore ) {
                        IE_ASSERT( alive( pFreeBefore ) == false );
                ...
                *freeOffset = offsetDeleted;
                ...
                IE_ASSERT( m_firstFreeOffset >= ElementSize );
                m_firstFreeOffset -= ElementSize;
    ...
    private:
        ...
        {
            IE_ASSERT( element != nullptr );
            IE_ASSERT( reinterpret_cast<const unsigned char*>( element ) >= reinterpret_cast<const unsigned char*>( m_memBegin ) &&
                    reinterpret_cast<const unsigned char*>( element ) <= reinterpret_cast<const unsigned char*>( m_memBegin ) + m_totalMemSize - ElementSize );
            IE_ASSERT( get_membyteoffset( element ) % ElementSize == 0 );
        ...
        {
            IE_ASSERT( element != nullptr );
            IE_ASSERT( reinterpret_cast<unsigned char*>( element ) >= reinterpret_cast<unsigned char*>( m_memBegin ) &&
